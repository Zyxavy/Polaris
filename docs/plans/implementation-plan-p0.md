...
**Docs:** ADR 001 §5.10, `cicd-deploy.md` §9.1 (first-time setup checklist), `cicd-deploy.md` §2.2–2.3
...
**Docs:** ADR 001 §5.10 (project structure), §5.2 (SvelteKit CSR), §5.3 (Hono)
...
3. Scaffold `packages/web` (SvelteKit, CSR-only):
   ...
   Then per `sveltekit-route-architecture.md` §8.3, install `@sveltejs/adapter-static` and configure it exactly as shown there (`fallback: 'index.html'`). Set `export const ssr = false; export const prerender = false;` in the root `+layout.ts` per §1.
...
5. Root `package.json` scripts per `cicd-deploy.md` §3.1:
...
6. Vitest in both packages (empty config for now, real tests start Slice 2) per `testing-strategy.md` §2.1–2.2.
7. Vite dev proxy in `packages/web/vite.config.ts` per `sveltekit-route-architecture.md` §8.1.
...
**Docs:** D1 Schema ADR (002) — full document is your source of truth here. `testing-strategy.md` §2.2 for the integration test setup.
...
### Migration files to create (in this order, per D1 Schema §6.2)
...
**Branch:** `feat/auth`
**Docs:** `auth-integration.md` (full doc), ADR 001 §5.6, `security-review.md` §1 and §4.

This is the first slice with a real end-to-end vertical: sign-up → session → sign-in → sign-out, plus the recovery-code fallback (since there's no email provider, per `auth-integration.md` §5).

### Backend tasks

1. `pnpm add better-auth` in `packages/api` (pre-approved dep per AGENTS.md). Confirm the current D1 adapter package name/import against Better Auth's own docs before writing `auth.ts` — the doc explicitly flags this as having moved between versions; don't trust the snippet in `auth-integration.md` §1.1 blindly, verify it.
2. `packages/api/src/auth.ts` — `createAuth(env)` factory per §1.1.
...
4. Mount `/api/auth/*` in `packages/api/src/index.ts` per §1.2.
5. `requireAuth` middleware per §1.3, applied to all of `/api/*` except `/api/auth/*`.
6. `recovery_codes` table already exists from Slice 2 (migration `0013`) — implement the sign-up flow's code generation (`POST /api/recovery-codes/generate`) and display (`GET /api/recovery-codes`) per §5.2. Code format `POLARIS-XXXX-XXXX` via `crypto.randomUUID()` truncation — no external dependency.
7. `POST /api/auth/recover` custom route per §5.2, registered **before** the Better Auth catch-all — copy the handler logic from §5.2 (email lookup → recovery code match → mark used → hash new password via Better Auth's exported hash function → update `account.password`).
8. Rate-limiting: per `security-review.md` §1, no lockout counter is required for v1 (risk accepted) — don't build one now, it's explicitly deferred.

### Frontend tasks

1. `packages/web/src/lib/auth-client.ts` per §4.1.
...
3. `(auth)/+layout.svelte` — auth guard redirecting signed-in users to `/guides` per `sveltekit-route-architecture.md` §3.4. (`/guides` itself doesn't need to exist yet with real content — a placeholder page satisfies this slice; full Guides content can be a small follow-up, it's static and low-risk.)
4. `(app)/+layout.ts` auth guard per §3.3 — redirects to `/sign-in` if no session. You'll need at least a placeholder `(app)/+layout.svelte` nav shell (can be minimal — full NavBar polish is fine to defer to Slice 11).
5. `apiFetch` wrapper (`packages/web/src/lib/api/client.ts`) per `sveltekit-route-architecture.md` §6 — every future slice's frontend depends on this existing correctly now, including `credentials: 'include'`.
...
- **E2E (Playwright, P0 flow #1 from `testing-strategy.md` §3.3):** sign up → lands on `/guides` → sign out → sign in → lands on `/dashboard` (dashboard can 404/placeholder at this point, just confirm the redirect target).

### Definition of Done

- [ ] All auth integration tests pass against real D1 (Miniflare).
- [ ] CSRF/`trustedOrigins` verified end-to-end in dev (sign up via the Vite proxy, confirm cookie is set — `auth-integration.md` §3 flags this as an easy-to-miss dev-only failure).
...
**Docs:** PRD §5.1, §6.1 (flow 1 only — "from scratch"; skip flows 2/3, templates and AI), `api-routes.md` §2, D1 Schema §5 (autosave/`floor_action` resolution), `design-system/polaris/pages/system-creator.md` (minus the Template Picker and AI Draft panel sections — don't build those yet), `sveltekit-route-architecture.md` §7.2.
...
2. `packages/api/src/lib/ownership.ts` — `getOwnedSystem` helper per `api-routes.md` §1.5 (Systems is the one resource that's directly `user_id`-scoped, but you'll reuse this ownership pattern in every later slice, so build it properly now).
3. Response envelope and error shape exactly per §1.3/§1.4 (`{error, message}`, `barrier_list` returned as parsed JSON array not raw string).
...
2. Debounced autosave: first tick calls `POST /api/systems` (no id yet), subsequent ticks call `PATCH /api/systems/:id`, per `api-routes.md` §2.2 and `system-creator.md`'s autosave section. Export `AUTOSAVE_DEBOUNCE_MS` as a named constant (testing-strategy.md §4.1 requires this for fake-timer tests).
...
4. "Confirm system" button calls `POST /api/systems/:id/confirm`, surfaces `floor_action_required` (422) as an inline field error per `loading-states.md` §2.3.
...
6. `/systems/[id]/+layout.ts` + `+layout.svelte` (tab shell) and `/systems/[id]/+page.svelte` (Overview tab) per `sveltekit-route-architecture.md` §2.2 and `design-system/polaris/pages/system-detail.md`. Workspace/Reviews tabs can 404/placeholder — those are Slices 8 and 10.
7. `/systems/[id]/edit/+page.svelte` reusing `SystemForm` pre-filled, per `system-edit.md`.

### Tests

- **Unit:** autosave debounce (fake timers, per `testing-strategy.md` §4.1 exactly). Form validation (name required).
- **Integration:** System CRUD full cycle (create → read → update → archive) per `testing-strategy.md` §3.2's "System CRUD" bullet. `POST .../confirm` rejects empty `floor_action` with 422, accepts non-empty.
...
**Docs:** PRD §5.2, D1 Schema §3.2, `api-routes.md` §3, `component-inventory.md`'s `SchedulePicker.svelte` spec.

Small, focused slice — wires the stub from Slice 4 into something real.

### Backend

1. `packages/api/src/routes/schedules.ts`: `GET`/`POST /api/systems/:system_id/schedules`, `PATCH`/`DELETE /api/schedules/:id`, all ownership-scoped through `systems.user_id` per §1.5.
2. Validate `time_window_end > time_window_start`, return `422 invalid_window` on failure per §3.
...
- **Unit:** `dayMatchesBitmask(dateStr, bitmask)` for all 7 days, edge case bitmask `0` (no days). This is a pure function per `testing-strategy.md` §7.3 — no D1 needed.
...
**Docs:** PRD §5.3, §6.3, `api-routes.md` §4.1–4.4 (the generation-logic SQL is copy-this-closely, it's already CPU-budget-optimized), `design-system/polaris/pages/dashboard.md`, `loading-states.md` (Dashboard rows).

This is the highest-value P0 slice — the daily-use loop.

### Backend

1. `packages/api/src/services/instances.ts` — `generateTodayInstances` per `testing-strategy.md` §7.3's example, using the exact 3-step SQL pattern from `api-routes.md` §4.1 (bitmask match in SQL, `D1.batch()` for inserts, no JS loop over rows). Uses `dayMatchesBitmask`... actually note: the SQL does the bitmask match directly (`sch.days_of_week & ? != 0`), so you don't call the JS helper here — that helper is for the Cron job's date math and any place that can't push it into SQL. Don't reintroduce a JS loop by mistake.
2. `toManilaDate(utcDate)` / a "today in Manila" helper in `packages/api/src/lib/calendar.ts` — unit-tested independently per `testing-strategy.md` §3.1.
3. `GET /api/dashboard` route: run lazy generation, then the filtered SELECT (window-gated) per §4.1. Catch `UNIQUE constraint failed` from a duplicate insert attempt and treat as no-op, not a 500 (per D1 Schema §3.3's note).
4. `GET /api/instances/:id`, `PATCH /api/instances/:id` (state transitions, `pending` never a valid target value) per §4.2–4.3.
5. `GET /api/systems/:system_id/instances` (paginated) per §4.4 — needed for the System Detail streak view and later for Reviews.

### Frontend

1. `dashboard-store.ts` (Svelte 5 runes class) exactly per `sveltekit-route-architecture.md` §5.2 — optimistic update with rollback on `markState`.
2. `InstanceList` / `InstanceCard` / `StateButtons` per `component-inventory.md` and `dashboard.md`'s bento grid + status header spec.
3. Dashboard skeleton loader per `loading-states.md` §1.2 (bento card rectangles, no full-page spinner).
4. Empty state ("Set up your first system") per `loading-states.md` §3.2.
...
- **Unit:** `toManilaDate` conversion across a UTC-midnight boundary (per `testing-strategy.md` §4.4 — pin `vi.setSystemTime()` away from midnight for other tests, but write one test specifically crossing it).
- **Integration:** idempotency test exactly as specified in `testing-strategy.md` §4.3 (call `generateTodayInstances` twice, assert no duplicate). Instance state transition test (`pending → full`, `updated_at` changes). Window-gated filter test (a system whose window hasn't opened yet is generated but excluded from the Dashboard response).
...
- [ ] 10ms CPU note: confirm via `wrangler tail` CPU field on a real/local call, note the ms in the PR (Definition of Done §5's explicit ask, not optional for this route).
...
**Docs:** ADR 001 §5.8, PRD §5.3 (path 2), `testing-strategy.md` §4.3 (Cron idempotency test), `cicd-deploy.md` §2.2 (`[triggers] crons = ["0 15 * * *"]`).
...
- [ ] Confirmed the lazy path (Slice 6) still works standalone if you simulate the Cron never having run — this is the "safety net" property ADR 001 §5.8 requires, worth an explicit test or at least a manual check.
- [ ] `wrangler tail` shows a `[cron]`-tagged log line locally (per `observability.md` §3.3/§4.2) — add the logging convention now, don't defer it, since Slice 6 should already have `[api]`/`[d1]` prefixes established.
...
**Docs:** PRD §5.4–§5.5, D1 Schema §3.3.1 + §3.4, `api-routes.md` §5–§6.1/6.2/6.3, `design-system/polaris/pages/workspace-builder.md`, `component-inventory.md` (Workspace components).
...
1. `PUT`/`GET /api/systems/:system_id/workspace` per §5 — full-layout replace, server-side `layout.v` enforcement (`upgradeLayout()` stub is fine now since only `v: 1` exists; write the function so it's a no-op today but has the right signature for future version bumps).
2. `POST /api/instances/:instance_id/counter-logs`, `GET /api/widgets/:widget_id/counter-logs`, `DELETE /api/counter-logs/:id` — §6.1.
3. Same pattern for timer-sessions — §6.2.
4. `PUT`/`GET /api/instances/:instance_id/checklist/:widget_id` — §6.3.
5. `workspace_snapshot` on Instance: compute-on-read per D1 Schema §3.3.1 (query the three logging tables `WHERE instance_id = ?`, don't try to keep it denormalized-and-synced from day one — that's an optimization, not correctness).
...
3. `workspace-editor-store.ts` (Svelte 5 runes class) exactly per `sveltekit-route-architecture.md` §5.3.
...
**Docs:** ADR 001 §5.5, MongoDB Schema ADR (003) in full, `api-routes.md` (Log widget isn't separately listed there — it's the `widget_entries.entry_type = 'log_meta'` pointer pattern from D1 Schema §3.3.1, plus a Mongo-backed content route you'll add).
...
2. Wire `MONGODB_URI` secret (`wrangler secret put MONGODB_URI` for prod; `mongodb://localhost:27017/polaris` for local dev per MongoDB Schema §8 — start the Docker container from Slice 0 if you haven't yet).
3. `journal_entries` collection — apply the validator from MongoDB Schema §3.1 once, locally, via `mongosh`.
4. A Log widget content route (design it consistent with the Checklist/Notes `PUT`-per-widget pattern already established — something like `POST /api/instances/:instance_id/journal/:widget_id` for a new entry, `GET /api/systems/:system_id/journal/:widget_id?...` for history). **This route isn't specified verbatim in `api-routes.md`** — that doc's §10/§11 inventory doesn't list a Log-widget-specific route, which looks like a gap in the docs rather than an intentional omission (Log is explicitly a P0 widget in the PRD). Flag this to me when you reach this slice and I'll help you design the exact contract consistent with the rest of §6 before you build it, rather than guessing.
5. Write path per ADR 001 §5.5 exactly: attempt direct Mongo write → on success, write the `widget_entries` pointer row (`entry_type: 'log_meta'`, `data: {"mongo_id": "..."}`) to D1 → 200. On Mongo write failure → enqueue to `polaris-journal-retry` → 202, D1 pointer row is **not** written yet (it can't be, you don't have the Mongo `_id`) — the Queue consumer must write both the Mongo document *and* the D1 pointer row on successful retry. This two-write coordination on the retry path isn't fully spelled out in the docs either — work through it with me before implementing, since getting the pointer-row timing wrong breaks the "seam" MongoDB Schema §2 describes.
6. Queue consumer (`queue` export in the same Worker) per ADR 001 §5.5 steps 4–5, retries with backoff, dead-letter queue is inspected manually (no code needed for that part).
...
- **Unit:** the Mongo write path is mocked at the driver level (`vi.mock('mongodb')`) per `testing-strategy.md` §3.1/§4.2's Workers AI mocking pattern (same idea, different dependency) — Mongo isn't emulated by Miniflare, confirmed by `testing-strategy.md` §3.2/§6.
...
- [ ] Manual verification of the retry path documented in the PR description (screenshot or `wrangler tail` log excerpt) — this is explicitly one of the few things the test suite can't cover per `testing-strategy.md` §6.
- [ ] `[mongo]`-tagged log lines present per `observability.md` §3.3.
- [ ] Update `api-routes.md` with the Log widget route contract you and I settle on — this is a real gap in the current doc, closing it satisfies Definition of Done §6/§7 (cross-doc consistency).
...
**Branch:** `feat/reviews`
**Docs:** PRD §5.7, §6.4, D1 Schema §3.6, `api-routes.md` §8, `design-system/polaris/pages/per-system-review.md` and `review-day.md`.

This is the slice that "closes the loop" per the product's core thesis — don't rush it.

### Backend

1. `POST /api/systems/:system_id/reviews` per §8.1 — the two-table write (insert `reviews` row, then conditionally `UPDATE systems` from `change_applied`). Implement as a `services/reviews.ts` function per `testing-strategy.md` §7.3 Rule 2 (cross-table write-back = service-layer logic, not inline route code).
...
3. `change_applied` derivation: structured object in → human-readable text out, with `change_applied_note` override, exactly per §8.1's format note.
4. `GET /api/systems/:system_id/reviews` (paginated).
5. `GET /api/review-day` per §8.2 — "due" computed server-side, no filter by the user's designated review day (convenience view, not a lock).
...
5. 409 handling: inline banner per `loading-states.md` §2.3's table ("A review already exists for this period").

### Tests

- **Integration:** review write-back test exactly as specified in `testing-strategy.md` §3.2 ("POST a Review with `change_applied` containing a new `floor_action`; assert the parent System's `floor_action` field is updated"). Duplicate-period 409 test.
...
1. Full `NavBar.svelte` (floating pill mobile, sidebar option desktop) per `MASTER.md` and `layout-specs.md` §3.
2. `ToastContainer.svelte` wired to `toast-store.ts` per `sveltekit-route-architecture.md` §5.4 — confirm every mutation route that should toast on error actually does (cross-check against `loading-states.md` §2.3's per-page table).
...
4. Sweep every page against `loading-states.md` §4 (the per-page state matrix) — confirm skeleton/error/empty states exist where the matrix says they should, not just the happy path.
...
1. `.github/workflows/ci.yml` exactly per `cicd-deploy.md` §4.2 — matrix (`lint`/`test:unit`/`build` per package) → `integration` → `e2e` → `deploy` (main-only, sequential: migrations → API → web).
2. Add `CLOUDFLARE_API_TOKEN` to GitHub Actions repo secrets now (this was deliberately deferred from Slice 0).
3. First manual deploy per §9.1's checklist (you're doing this for real now, not just locally) — `pnpm -r build`, `pnpm -r deploy`, verify sign-up works against the live `*.workers.dev` URLs.
4. Push to `main` once, confirm the CI `deploy` job runs end-to-end.

### Definition of Done

- [ ] CI matrix, integration, and e2e jobs all green on a real PR.
- [ ] Manual first deploy succeeded and was verified against the live URL (§9.1's exact verification step: sign up, create a system, see it on the dashboard).
...
**Docs:** `definition-of-done.md`, `security-review.md` §1–§3 (skip §4, already covered in Slice 3), `disaster-recovery.md` §1.1 (manual D1 backup — run it once now that there's real data worth backing up).
...
2. `security-review.md` §2 (R2 attachment validation) — **not applicable yet**, since attachments are P1 and no upload route exists. Confirm this explicitly rather than silently skipping.
3. `security-review.md` §3 (XSS in freeform fields) — audit every place `philosophy`/`purpose`/`protocol`/`barrier_list`/Log text/Review fields render, confirm none use `{@html}`.
4. Run a manual D1 backup per `disaster-recovery.md` §1.1 for the first time against production data, confirm the R2 upload succeeds, note the cadence going forward (weekly-ish).
5. Enable GitHub's free Dependabot alerts (Settings → Code security) per `security-review.md` §5 — a one-time repo setting, not code.
