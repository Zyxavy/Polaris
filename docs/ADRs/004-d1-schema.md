...
Every D1 table used by the API Worker, in exact `CREATE TABLE` form, plus the indexes and constraints needed to support the query patterns in the PRD (dashboard load, nightly Cron generation, review aggregation) and the idempotency guarantees in the [Testing Strategy](002-testing-strategy.md) (S4.3).
